...
def _header(spec: MapSpec, errors: List[str]) -> str:
    ...
    for e in errors:
        lines.append(f"# TODO (spec issue): {e}")
    return "\n".join(lines)
...
def _main_block(spec: MapSpec) -> str:
    ...
    lines.append("    m = aprx.listMaps()[0] if aprx.listMaps() else aprx.createMap('Map')")
    lines.append("    for _old in list(m.listLayers()):")
    lines.append("        if not _old.isBasemapLayer:")
    lines.append("            m.removeLayer(_old)")
    lines.append("")
